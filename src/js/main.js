...
$(function(){
	...
	if("object" == typeof window.CatalogAvon){
			...
			loadJsonFileFn = function(data){
				...
				if("object" == typeof data.Pages){
					...
					for (j; j<pg.length; ++j){
						...
						/*widthCatalog = parseInt(580 * width / height);
						heightCatalog = 580;
						2 * widthCatalog > 1155 && (widthCatalog = parseInt(577.5));
						heightCatalog = Math.floor(585 * height / width);*/
						//$(".turnjs-slider").width(2 * widthCatalog);
						//$("#sliderbar").width(2 * widthCatalog - 30);
						pages.push({
					...
					if(fid.length){
						...
						$.ajax({
							...
							success: function(d){
								...
								$.each(d, function(){
									tpl += "<div class=\"ProductItem container\">";
										tpl += "<div class=\"ProductItemDiv\">";
											tpl += "<div class=\"ProductItemImage ProductItemImageMd hidden-xs hidden-sm col-md-4\">";
												tpl += "<span class=\"ProductItem_price\">"+this.sale+"</span>";
												tpl += "<img class=\"img-responsive\" src=\""+window.avonCdn+this.thumb+"\" data-href=\""+window.avonCdn+this.image+"\" data-position=\""+pi+"\" width=\"310\" height=\"310\" alt=\""+this.name+"\" data-sale=\""+this.sale+"\" />";
											tpl += "</div>";
											tpl += "<div class=\"ProductItemText col-md-8\">";
												tpl += "<h4 class=\"ProductItem_title\">"+this.name+"</h4>";
												tpl += "<div class=\"ProductItemImage ProductAvonImage visible-xs visible-sm\">";
													tpl += "<span class=\"ProductItem_price\">"+this.sale+"</span>";
													tpl += "<img class=\"img-responsive\" src=\""+window.avonCdn+this.thumb+"\" data-href=\""+window.avonCdn+this.image+"\" data-position=\""+pi+"\" width=\"310\" height=\"310\" alt=\""+this.name+"\" data-sale=\""+this.sale+"\" />";
												tpl += "</div>";
								...
								if(pi!=0){
									tpl = "<div class=\"ProductsWrapper\">"+tpl+"</div>";
		...
		$(".catalogouter_wrapper_products").on("click", ".ProductItemImage img, .ProductItemImageSm img", function(e){
			e.preventDefault();
			...
			$.each(imgs, function(){
				data.push({
					...
					opts: {
						caption: $(this).attr("alt")+"<br>"+$(this).attr("data-sale")
					}
				});
			})
			$.fancybox.open(data, {
